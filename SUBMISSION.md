...
| T20  |     |          | Replaced Nginx with an Express server in Dockerfile to handle server-side Google OAuth securely without exposing `GOOGLE_CLIENT_SECRET` to the client. Provided `/auth/me` to safely verify session. |
